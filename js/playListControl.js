...
export class PlayListControl{
    ...
    async init(){
        ...
        this.getAllPlayList().forEach((playList,index) => {
            var insertHtml = `<li class="li-playList" id=${playList.id} keyIndex=${playList.key}>
                                <div class="li-order" id=${playList.id} keyIndex=${playList.key}>
                                <div class="disc">
                                ...
                                <div class="li-delete fade-out" id='${playList.id}' keyIndex=${playList.key}>
        ...
        [...this.deleteIcons].forEach((deleteIcon) => {
            deleteIcon.addEventListener('click',()=>{this.deleteMusic(deleteIcon)});
        });
        this.orderControl = new OrderControl(this.getAllPlayList(), this.getAudioControl());
    ...
    async deleteMusic(deleteIcon){
        const deleteResult = await this.indexedDB.playListDelete(Number(deleteIcon.getAttribute('keyIndex')));
        if(deleteResult === 200){
            this.getAllPlayList();
            this.init();
            var all = await this.indexedDB.getAllIndexedPlayList();
